...
$(function() {
		...
		if(parents_name == "") {
			...
			layer.open({
				...
				yes: function(index, layero) {
					...
					$.ajax({
						type: type,
						url: org_url + dataUrl.parent + "?token=" + sessionStorage.token,
						...
						success: function(data) {
							...
							if (data.code==20004) {
								...
								layer.alert('新建家长成功！',function(index){
	...
	function initStudent(panel, student) {
		...
		$.ajax({
			type: "get",
			url: org_url + dataUrl.studentbyclazz,
			async: true,
			dataType: "json",
			data: {
	...
	function load(id) {
		$.ajax({
			type: "get",
			url: org_url + dataUrl.parentsbystudent + id + "/students?pid=" + id + "&token=" + sessionStorage.token,
			async: true,
			dataType: "json",
			xhrFields: {
				withCredentials: true
